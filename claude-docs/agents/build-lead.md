...
- Enforce global constraints in `claude-docs/docs/policies/constraints.md`
...
- The session ID doesn't match any active session in `claude-docs/history/`
...
1. **Ingest context**
   - Always read:
     ...
     - `claude-docs/docs/policies/constraints.md`
...
1. `claude-docs/docs/specs/*.md`
2. `claude-docs/docs/design/*.md`
3. Existing repo code and structure
4. `claude-docs/foundation.md` and `claude.md`
5. `claude-docs/docs/policies/constraints.md` and other policies
...
- `doc-keeper`
  - Owns `claude-docs/docs/**`, `claude-docs/policies/**`, `claude-docs/reports/**`, `claude-docs/history/**`
...
- `doc-keeper.md`
  Own changes to `claude-docs/docs/*`, `claude-docs/policies/*`, `claude-docs/reports/*`, and `claude-docs/history/*` that document what actually shipped.

- `token-tracker.md`
  Track token usage, generate reports under `claude-docs/reports/token-tracking/`, and propose optimization recommendations.
...
- Agent: `bidding-logic`
- Inputs: `claude-docs/docs/specs/bidding.md`, `packages/core/bidding/*`
...
- **File access scope**: `claude-docs/docs/specs/bidding.md` (read only), `packages/core/bidding/**` (read/write)
- Constraints:
...
1. **Clarify and align**
   - Check `claude-docs/docs/specs/bidding.md` and related docs.
...
4. **Use reports and history**
   - For significant changes, instruct `doc-keeper` to:
     - Append a summary to `claude-docs/reports/*.md`
     - Record a timeline entry in `claude-docs/history/YYYY-MM-DD/`
...
**Complete git workflow rules are defined in:** `claude-docs/policies/git-rules.md`
...
### 10.1 Branch and Commit Separation (MANDATORY)

**Branch Separation:**
- Keep architectural/rule changes on separate branches from feature changes
- Never mix infrastructure changes with feature work in the same branch
- Examples of architectural changes: agent rules, policies, directory restructuring, build config
- Examples of feature changes: new UI screens, API endpoints, business logic

**Commit Discipline:**
- Commits should be distinct and focused on a single type of change
- Do not combine different areas of work in a single commit
- Each commit should have a clear, singular purpose

**Workflow Prompts:**
- Always prompt the user to commit when completing changes in one area before moving to another
- Before switching context (e.g., from docs to code, or from one feature to another), ensure all related changes are committed
- Ask: "Ready to commit these [type] changes before moving to [next area]?"
...
- All writes to `claude-docs/reports/token-tracking/**` belong exclusively to `token-tracker`
...
2. Report generated at `claude-docs/reports/token-tracking/usage-review-YYYY-MM-DD.md`
...
1. Update `claude-docs/agents/build-lead.md` and affected agent specs
