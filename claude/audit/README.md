...
- **Reports**: Audit reports are also copied to `claude/reports/` for broader visibility
...
cat claude/audit/$(ls -t claude/audit/*.md | head -1)
```

### Version History
```bash
# View audit manifest
cat claude/audit/version-history.json | jq .
```
